...
def analyse_grid_cell(ds: xr.Dataset, y_idx: int, x_idx: int,
                      nwmn_start_dt: datetime, nwmn_end_dt: datetime,
                      lunarperiod_d: float, tidalperiod_h: float) -> dict:
    """
    ...
    for z in zero_crossings:
        z_time = z['time'].replace(tzinfo=timezone.utc)
        moon_ctx = get_moon_cycle_context_direct(z_time, nwmn_start_dt, nwmn_end_dt, lunarperiod_d, tidalperiod_h)
        reversals.append({
            "time": z_time.isoformat(),
            "direction": z["direction"],
            "lunar_phase_deg": moon_ctx["lunar_phase_deg"],
            "nominal_tidal_phase_deg": moon_ctx["nominal_tidal_phase_deg"],
            "offset_days": moon_ctx["offset_days"]
        })
    # Neap/Spring Analysis over this period
    amplitudes = [abs(p["amplitude_mps"]) for p in peaks]
...
def plot_cell_analysis(ds: xr.Dataset, y_idx: int, x_idx: int, cell_analytics: dict, output_path: Path):
    """
    Generates a beautiful diagnostic visualization of the cell's current velocities,
    reversals, and peak flow points over the NetCDF time range.
    Saves the plot as a PNG for verification.
    """
    ...
    plt.plot(times, v_parallel, label="Coast-Parallel Current (Major Axis)", color="royalblue", linewidth=2)
    plt.plot(times, v_perpendicular, label="Rotary Transverse Current (Minor Axis)", color="mediumseagreen",
             linestyle="--", alpha=0.7)
    # Plot peaks
    peak_times = [pd.to_datetime(p["peak_time"]) for p in cell_analytics["peaks"]]
    peak_amps = [p["amplitude_mps"] for p in cell_analytics["peaks"]]
    plt.scatter(peak_times, peak_amps, color="crimson", zorder=5, s=40, label="Detected Peak Flow")
    # Plot slack/reversals
    reversal_times = [pd.to_datetime(r["time"]) for r in cell_analytics["reversals"]]
    for r_time in reversal_times:
        plt.axvline(r_time, color="orange", linestyle=":", alpha=0.5, zorder=1)
    if reversal_times:
        plt.scatter(
            reversal_times,
            [0]*len(reversal_times),
            color="orange",
            zorder=5,
            marker="x",
            s=50,
            label="Slack Water Reversal"
        )
    plt.axhline(0, color="gray", linewidth=0.8, linestyle="-", alpha=0.5)
    plt.title(f"Tidal Analysis Verification - Cell [{cell_analytics['latitude']:.5f}N, {cell_analytics['longitude']:.5f}E]\n"
              f"Principal Tidal Axis: {cell_analytics['principal_flow_angle_deg']:.1f}° True North", fontsize=12, fontweight="bold")
...
def process_nc_file(nc_path: Path,
                    nwmn_start_dt: datetime, nwmn_end_dt: datetime,
                    lunarperiod_d: float, tidalperiod_h: float,
    ...
    checks caching to skip if up-to-date, and generates json companion and visual validation plots.
    """
    ...
    # 4. Generate visual verification plot for selected focal coordinates (e.g. Koksijde start)
    if focal_positions:
        for f_idx, f_pos in enumerate(focal_positions):
            ...
            if cell_id in cells_analytics:
                plot_name = f"{nc_path.stem}_verify_focal_{f_idx+1}.png"
                plot_path = nc_path.parent / plot_name
                plot_cell_analysis(ds, y_idx, x_idx, cells_analytics[cell_id], plot_path)
            else:
                log.warning(f"Focal position closest cell {cell_id} was filtered out by geometry constraints. Plot skipped.")
    return out_json_path


# this has to be redone to use the new CMEMSDataManager catalog and its metadata
#  -- the ai code agent probably resorts to this cause it has no other way to instantiate the catalog and its metadata?
#  -- it should be instructed to mock the catalog and its metadata for testing purposes, not only the nc files
# also replaced the lat_lon tuples with the cscm.model.Position so we can directly reuse defined Points in wellknown.py 
# to have them as focal points for plots
def process_catalog(catalog: pd.DataFrame,
                    *,
                    geom_to_filter: BaseGeometry = None,
                    force_recalculate: bool = False,
                    focal_positions: list[Position] = None):
...
# TODO -- remove this -- relic from tests executed by notebooklm -- nonsensical outside that context
# not in sync with the local updates -- does not work as is
if __name__ == "__main__":
    ...
    if mock_nc.exists():
        ...
        koksijde_start = (51.11940, 2.62575)
        process_nc_file(
            nc_path=mock_nc,
