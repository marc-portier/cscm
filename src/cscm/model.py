...
class MovingObject(ABC):
    ...
    def env_compat_check(self, environment: Environment):
        ...
        Base implementation does nothing, but can be overridden by subclasses to check for specific environment types.
        Should raise an exception (ValueError) if not compatible.
        """
        pass
    ...
    def move(self, heading_deg: float, time_s: int, environment: Environment) -> Position:
        ...
        Should increase the internal lapsed time of the object by the given time, allowing it to keep track of 
        its state and its effect in subsequent moves.
        """
    ...
    def calculate_speed(self, heading_deg: float, time_s: int, environment: Environment) -> SpeedVector:
        """
        Calculates the effective speed vector of the object in the environment for a given time in seconds,
        with a given intended heading in degrees.
        Note the environment may affect the actual movement (direction and speed) of the object, e.g. by currents.
    ...
    def calculate_proper_speed_mps(self, heading_deg: float, time_s: int, environment: Environment) -> float:
        ...
        It should however not account for the factual displayment caused by the environment (like current),
        as this is done in the move() method.
        Returns the effective and intrinisic/proper (no currents!) speed (in meter per second) the object
        will average during this move.
        """
        pass
...
# something that uses some config (size of discrete time step, ...)
# to calculate a trajectory for a given moving object in a given environment model
class TrajectoryCalculator:
    """
    Base class for trajectory calculator.
    Provides a method to calculate a trajectory for a given moving object in a given environment model.
    """
...
# something that can find the shortest-time trajectory for a given moving object in
# a given environment model, given a start and end position
class TrajectoryFinder:
    # TODO unclear how it could be made to apply different strategies to scan the space of possible guiding heading_deg_fn 
    # -- could be simply scanning different straight line headings, or a fan out variant of that by recursively re-scanning 
    #    after X time_steps, or maybe even a genetic algorithm, etc.
    """
    System that can find the shortest-time trajectory for a given moving object in a given environment model,
    ...
    Note that the search space can be very large, so the implementation should be efficient and may need to use heuristics
    or approximations to find a good solution in a reasonable time.
    The search space can be defined by the user, e.g. by providing a range of start times and a range of heading_deg_functions
    to scan. The implementation should be able to handle different types of moving
    objects and environment models, and should be able to adapt to different scenarios and constraints.
    Scans space of possible start-times and a dynamically adjusted (generated) heading_deg_functions to calculate their
    corresponding trajectory and final time to reach a given end position.
    The best trajectory (shortest time) is returned.
    """
    def __init__(
            self,
    ...
    def find_optimum(
        ...
        :param heading_deg_fn: A function that returns the heading in degrees for a given time step.
        ...
        best_time = float('inf')
        start_time_range = pd.date_range(start=first_start_time_dt, end=last_start_time_dt,
                                         freq=timedelta(seconds=start_time_step_s))
        # TODO 
